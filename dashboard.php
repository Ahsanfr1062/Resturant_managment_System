...
<section class="bg-white dark:bg-gray-900 rounded">
    <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16">
        <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl lg:text-6xl dark:text-white">Check Your All stats Here!</h1>
...
require('partials/_connectdb.php');
  $sql = "SELECT * FROM `order_data`o, `products_data` p WHERE o.product_id = p.sno;";
...
<div class="p-4 flex items-center">
      ...
      <div>
        <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">Average Sale Price
        </p>
        <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
          '.$sumEarning/$totalOrders.'
        </p>
      </div>
    </div>
  </div>
  ';
  $sql = "SELECT * FROM `delivery`d , `products_data` p WHERE d.product_id = p.sno AND status = 'CANCELLED'";
  $NetEarning = 0;
  $cancelledEarning = 0;
  $result =  mysqli_query($conn, $sql);
  $cancelledOrders = mysqli_num_rows($result);
  while ($row = mysqli_fetch_array($result)){
    $cancelledEarning += $row['price'];
  }
  $NetEarning = $sumEarning - $cancelledEarning;
  echo '
  <div
  class="min-w-0 rounded-lg shadow-xs overflow-hidden bg-white dark:bg-gray-800"
  >
  ...
  <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">'.$cancelledOrders.'</p>
  </div>
  </div>
  </div>
  </div>';
 ...
 >
   ...
   <div
     ...
     <div>
       ...
       $sql = "SELECT DISTINCT EMAIL FROM `order_data`";
       $result =  mysqli_query($conn, $sql);
    ...
    <div class="p-4 flex items-center">
      ...
      <div>
       ...
       $result =  mysqli_query($conn, $sql);
       $pendingOrders = mysqli_num_rows($result);
